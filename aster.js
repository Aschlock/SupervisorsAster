...
// ==/UserScript==
    ...
    $(window).ajaxComplete( function(){ for ( var i = 0; i < document.getElementsByClassName('row').length; i++ ) {
...
$(window).ajaxComplete( function(){ for ( var i = 0; i < document.getElementsByClassName('row').length; i++ ) {
  if ( document.getElementsByClassName('row')[i].cells[1].innerText.indexOf("Сложный инцидент") !== -1  && document.getElementsByClassName('row')[i].cells[1].innerText >= 'Сложный инцидент (00:10:00)' )
    ...
    $(window).ajaxComplete( function(){ var elements = $('#agents_queues .tab:first-of-type .row') || [];
elements = [].slice.apply(elements);

elements.forEach(function (element, i) {
    element.addEventListener('click', function () {
if ( $('.toolbar').length == 0 ) { ultoolbar.insertAfter(element) } else {
...
$(window).ajaxComplete( function(){ for ( var i = 0; i < document.getElementsByClassName('row').length; i++ ) {
  if ( ( document.getElementsByClassName('row')[i].cells[2].innerText !== '[+2]' && document.getElementsByClassName('row')[i].cells[2].innerText !== '[+1]' ) && document.getElementsByClassName('row')[i].cells[1].innerText.indexOf("Почта") !== -1 )
    ...
    $(document).ready( function appensd() {$("#header").append("<img width='20px' title='Телефон супервизора' class='arrow' src='https://cdn3.iconfinder.com/data/icons/faticons/32/arrow-down-01-512.png'>")
